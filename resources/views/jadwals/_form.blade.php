...
<div>
    ...
    <select name="guru_id" class="mt-1.5 block w-full border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
...
<div>
    ...
    <select name="kelas_id" class="mt-1.5 block w-full border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        <option value="">-- Pilih Kelas --</option>
        @foreach ($kelas as $k)
            <option value="{{ $k->id }}" @selected(old('kelas_id', $jadwal->kelas_id ?? '') == $k->id)>{{ $k->nama_kelas }} ({{ $k->jenjang }} · {{ $k->semester }} {{ $k->tahun_ajar }} · {{ $k->jumlah_siswa }} siswa)</option>
        @endforeach
    </select>
...
<div class="grid grid-cols-2 gap-4">
    <div>
        ...
        <input type="text" name="mapel" value="{{ old('mapel', $jadwal->mapel ?? '') }}" placeholder="mis. Matematika"
    ...
    <div>
        ...
        <select name="hari" class="mt-1.5 block w-full border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
