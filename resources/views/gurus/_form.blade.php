...
<div class="grid grid-cols-2 gap-4">
    <div>
        ...
        <input type="text" name="no_hp" value="{{ old('no_hp', $guru->no_hp ?? '') }}" placeholder="0812xxxxxxx"
            class="mt-1.5 block w-full border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        @error('no_hp') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
    </div>
    <div>
        <label class="block text-sm font-medium text-gray-700">Status</label>
        <select name="status" class="mt-1.5 block w-full border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
...
<div>
    ...
    <textarea name="alamat" rows="2" class="mt-1.5 block w-full border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('alamat', $guru->alamat ?? '') }}</textarea>
